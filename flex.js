...
import {applyEnemySkillPose,getScrapBurrowPhase} from "./enemy/skill-presentation.js";
...
for(let id=1;id<ENEMY_TYPES.length;id+=1){
  ...
  group.traverse((child)=>{
    ...
    snapshots.push({child,position:child.position.clone(),quaternion:child.quaternion.clone(),scale:child.scale.clone(),opacity:child.material?.opacity});
  ...
  exhibits.push({id,type,typeId:id,group,parts,flying,nameLabel,speed:type.speed,seed:0,snapshots,home:new THREE.Vector3(x,.08,z),anchor:new THREE.Vector3(x,.08,z),groupScale:group.scale.clone(),baseScale:group.scale.clone(),animationBaseY:0,deathBaseY:.08,deathBaseRotationY:0,deathDuration:1.2});
}
...
function previewTone(frequency,duration=.1,volume=.08,wave="triangle",endRatio=1,delay=0){
  ...
  filter.type="lowpass";filter.frequency.value=Math.min(4200,Math.max(280,frequency*8));
...
function previewNoise(duration=.1,volume=.05,delay=0){
  if(!ensurePreviewAudio())return;
  ...
  const source=previewAudioContext.createBufferSource(),gain=previewAudioContext.createGain();source.buffer=previewNoiseBuffer;
  gain.gain.setValueAtTime(volume,previewAudioContext.currentTime+delay);gain.gain.exponentialRampToValueAtTime(.0001,previewAudioContext.currentTime+delay+duration);
  source.connect(gain).connect(previewAudioGain);source.start(previewAudioContext.currentTime+delay,0,duration);
}
...
function animateSkillIndicator(exhibit,definition,time){
  ...
  if(definition.skill.handler==="scrapBurrow"&&time>=3){
    kind="scrap-emerge";age=time-3;options={...options,kind,radius:.75,life:1.05};key=`${definition.id}:${kind}`;
...
function updatePreviewAudio(preview,totalElapsed,elapsed,cycleDuration){
  ...
  if(cycle!==preview.audioCycle){
    ...
    else if(action==="stunned")playPreviewEnemySound(exhibit,"hurt");
...
function updateAnimationPreview(now){
  ...
  const cycleDuration=action==="idle"?animation.idleDuration:action==="walk"?animation.locomotionDuration:action==="attack"?animation.attackDuration:action==="skill"?animation.skillDuration:action==="stunned"?animation.stunnedDuration:exhibit.deathDuration;
  ...
  if(action==="idle")applyEnemyPose(exhibit,{elapsed});
    ...
    const progress=elapsed/cycleDuration;
    const attackStrength=progress<.2?0:Math.exp(-(elapsed-cycleDuration*.2)*4.5);
    applyEnemyPose(exhibit,{elapsed,attackStrength});
    if(exhibit.type.range<=2&&progress>.58)animateDummyHit((progress-.58)/.42);
